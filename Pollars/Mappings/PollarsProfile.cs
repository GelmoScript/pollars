...
namespace Pollars.Mappings
{
    public class PollarsProfile : Profile
    {
        public PollarsProfile()
        {
            this._CreateMap_WithConventions_FromAssemblies<User, UserDto>();
            this._CreateMap_WithConventions_FromAssemblies<Question, QuestionDto>();
            this._CreateMap_WithConventions_FromAssemblies<Poll, PollDto>();
        }
    }
    static class MapperExtensions
    {
        public static Profile _CreateMap_WithConventions_FromAssemblies<T, TDto>(this Profile profile, bool reverseMap = true) where T : class where TDto : class, IBaseDto
        {
